  ...
  <section class="app-list-wrap">
    ...
    <section class="filter-wrapper" @keyup.enter="searchBtn">
      <div class="filter-line">
        ...
        <!-- 应用状态-->
        <div class="select-wrapper">
          <Select v-model="params.appState" clearable>
            <Option v-for="item in params.appStates" :value="item.id" :key="item.id">{{item.name}}</Option>
          </Select>
        </div>
      </div>
    </section>
    ...
    <section id="nav-tab">
      <Tabs active-key="key1">
        <Tab-pane label="全部应用(108)" key="key1"></Tab-pane>
        <Tab-pane label="已上架(3)" key="key2"></Tab-pane>
        <Tab-pane label="未上架(27)" key="key3"></Tab-pane>
      </Tabs>
    </section>
    <!--过滤条件-->
    <section class="filter-wrapper" @keyup.enter="searchBtn">
    ...
    <section class="list-wrapper">
      <custom-table :thead="thead" :tbody="tbody" :scroll="true" :selectAll="selectAllFlag" @selectRecord="selectRecord" @selectAllRecord="selectAllRecord">
